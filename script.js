...
const apiKey = "58f7fb990a4e22c021b8a1c65595f862";
...
function showNotification(notification_text) {
    ...
    message.textContent = notification_text;
...
function reset()
{
	...
	document.querySelector('#fog').style.backdropFilter = 'blur(0px)';
}
...
function makeCloudy() {
    reset(); // Reset previous weather styles and settings
    container.addClass('cloudy'); // Add the "cloudy" class to the container
    currentWeather = weather.find(w => w.type === 'cloudy'); // Update the current weather
    updateSummaryText(); // Update the weather summary text

    // Modify cloud movement for "cloudy" weather
    for (var i = 0; i < clouds.length; i++) {
        clouds[i].offset = Math.random() * sizes.card.width; // Reset cloud positions randomly
        TweenMax.to(clouds[i].group.node, 2, {
            opacity: 1, // Make clouds visible
            ease: Power1.easeInOut
        });
    }

    // Hide elements not needed for cloudy weather
    TweenMax.to(sun.node, 1, { opacity: 0 }); // Hide the sun
    TweenMax.to(sunburst.node, 1, { opacity: 0 }); // Hide the sunburst
    document.querySelector('#fog').style.backdropFilter = 'blur(0px)'; // Ensure fog is hidden

    // Adjust settings for cloudy weather
    settings.windSpeed = 1; // Reduce wind speed for clouds to appear static
    startLightningTimer(); // If needed, allow for occasional lightning
}
...
function makeFog() {
	...
	document.querySelector('#fog').style.backdropFilter = 'blur(3px)';
}
...
function changeWeather(weather)
{
	...
	// Apply specific weather behaviors
    switch (weather.type) {
		case 'fog':
			makeFog();
			break;
        case 'snow':
            ...
            break;
        case 'rain':
            settings.rainCount = 30;
            break;
        case 'thunder':
            settings.rainCount = 60;
            startLightningTimer();
            break;
        case 'cloudy':
            makeCloudy();
            break;
        case 'sun':
            settings.windSpeed = 2;
            TweenMax.to(sunburst.node, 1, { opacity: 0.5 });
            break;
    }
	...
	switch (weather.type) {
		...
		// Other weather types
		case 'rain':
			settings.rainCount = 10;
			break;
		case 'snow':
			settings.snowCount = 10;
			break;
		case 'sun':
			TweenMax.to(sunburst.node, 4, {opacity: 0.8, ease: Power1.easeInOut});
			break;
		case 'wind':
	...
	if (navigator.geolocation) {
		navigator.geolocation.getCurrentPosition(
			(position) => {
				...
				const apiUrl2 = `https://api.openweathermap.org/data/2.5/weather?units=metric&lat=${latitude}&lon=${longitude}&appid=${apiKey}`;
				...
				async function checkWeather() {
					try {
						...
						document.querySelector("#summary").innerHTML = data.weather[0].main;
						...
						switch (data.weather[0].main) {
							...
							case "Snow":
								...
								break;
							default:
								weatherName = "Cloudy"; // Default to Cloudy
								break;
						}

						reset();
						// Find the matching weather object
						const weatherObj = weather.find(w => w.name === weatherName);
						if (weatherObj) {
							changeWeather(weatherObj);
						} else {
							changeWeather(weather[0]); // Default weather if not found
						}

					} catch (error) {
						console.error("Error fetching weather data:", error.message);
						// alert("Failed to fetch weather data.");
						notification_text = "Failed to fetch weather data!!";
			...
			(error) => {
				...
				switch (error.code) {
					case error.PERMISSION_DENIED:
						...
						notification_text = "User denied the request for Geolocation!! Please turn on location access in your Browser!!";
						showNotification(notification_text);
						const apiUrl = "https://api.openweathermap.org/data/2.5/weather?units=metric&q=";
var searchBox = document.querySelector(".input");
var send = document.querySelector("#filter-icon");
...
async function checkWeather(city) {
    try {
		...
		notification_text = "Enter Valid Location!!";
		showNotification(notification_text);
    }
}
const city = "New Delhi"; // Default city
checkWeather(city);

						break;
					case error.POSITION_UNAVAILABLE:
						console.error("Location information is unavailable.");
						notification_text = "Location information is unavailable!!";
						showNotification(notification_text);
						break;
					case error.TIMEOUT:
						console.error("The request to get user location timed out.");
						notification_text = "The request to get user location timed out!!";
						showNotification(notification_text);
						break;
					default:
		...
		notification_text = "Geolocation is not supported by this browser!!";
		showNotification(notification_text);
	}
}
...
function getCurrentDate() {
	...
	var currentDate = `${day} ${month} ${dayName}`;
